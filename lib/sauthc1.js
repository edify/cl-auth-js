...
function canonicalizeQueryString(queryParams) {
    ...
    let queryStringList = _.map(queryParamsNames, function(paramName) {
        encodedName = encodeURL(paramName, false, true);
        paramValue = queryParams[paramName];
        encodedValue = encodeURL(paramValue, false, true);
 ...
 */
function canonicalizeResourcePath(path) { return path.length === 0 ? '/' : encodeURL(path, true, true) }
